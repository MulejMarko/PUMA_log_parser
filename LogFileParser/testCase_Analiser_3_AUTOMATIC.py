...
class transition_switch(object):
    ...
    def analise_AUTOMATIC_MONITOR(self):
        '''
        Analise the transition from AUTOMATIC to MONITOR
        '''
        phase_continuie = True
        ...
        for fields in buffer[start_line:]:
            line += 1
            if any(am in fields[3] for am in ['MANUAL']):
                break
            if any(i in fields[0] for i in ['(A)', '(E)']):
                error_log.append(fields)
    ...
    def analise_EndOfTest(self):
        ...
        start_line = self.lastPrecondition_line
        buffer = self.current_buffer
        analises_log.append(['', buffer[start_line][1], 'Switcher: analise_EndOfTest triggered.'])
...
for test_case in tests:
    ...
    for file in dir_log_files(test_log_dir):
        ...
        try:
            A_test = False
            for fields in current_buffer:
                if 'AUTOMATIC' in fields[3]: A_test = True
            if not A_test: raise Exception
        except:
            _index -= 1
            continue
    ...
    # because Robert wants to only see relevant logfiles I had to build in this function 
    with open(o_filename, 'rt') as r_input:
        r_read = csv.reader(r_input, delimiter=';')
        r_buffer = list(r_read)
    r_input.close()
    if len(r_buffer)<4: os.remove(o_filename)  # remove empty report

try: 
    os.remove('M:\\Projects\\_Stubler_EOL_logs\\Result_Analises\\None.csv')
except:
    pass
